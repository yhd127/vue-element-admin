...
export default {
  ...
  methods: {
    ...
    handleInputChange(prop, row) {
      ...
      // 检测特定参数的变化 (支持中英文参数名)
      if (row.paramName === 'Direction' || row.paramName === '方向') {
        // 方向参数变化时，触发更新
        this.$emit('direction-changed', row.value)
      } else if (row.paramName === 'Line speed' || row.paramName === '线路速度') {
        // 线路速度变化时，触发更新
        this.$emit('line-speed-changed', row.value)
      } else if (row.paramName === 'Max PSR' || row.paramName === '最大PSR') {
        // 最大PSR值变化时，触发更新
        this.$emit('max-psr-changed', row.value)
      } else if (row.paramName === 'Train length' || row.paramName === '列车长度') {
        // 列车长度变化时，触发更新
