 ...
 * 计算PSR表中PSR_ms字段的值
 * @param {Object} row 当前行数据
 * @param {Number} maxPSR 最大PSR值，用于处理文本PSR
 * @returns {Number|String} 计算结果，PSR值从km/h转换为m/s
 */
export function calculatePSRMs(row, maxPSR = 350) {
  const psrValue = row.PSR

  // 如果PSR是文本类型（不能转换为数值）
  if (psrValue !== '' && psrValue !== undefined && isNaN(Number(psrValue))) {
    return (maxPSR / 3.6).toFixed(2)
  }

  // 如果PSR为空，尝试返回前一行的值（仅在索引信息可用时）
  ...
  return (numericPSR / 3.6).toFixed(2)
}
 ...
 * @param {Array} tracksData Tracks表数据
 * @param {Number} direction 方向参数
 * @param {Number} maxPSR 最大PSR值，用于计算PSR_ms
 */
export function initializePSRCalculations(psrData, tracksData, direction, maxPSR = 350) {
  ...
  for (let i = 0; i < psrData.length; i++) {
    ...
    row.PSR_ms = calculatePSRMs(row, maxPSR)
  }
}
