  ...
  methods: {
    ...
    cellClassName({ row, column }) {
      ...
      if (this.activeSheet === 'Tracks') {
        ...
        if (property === 'Track_ID_after_jump') {
          ...
          console.log('DEBUG - 标红逻辑:', {
            row: row,
            property: property,
            kpBeforeJump: kpBeforeJump,
            kpAfterJump: kpAfterJump,
            hasKpJump: hasKpJump,
            beforeJumpID: beforeJumpID,
      ...
      if (this.activeSheet === 'PSR' && property === 'Track1') {
        // 检查同一行的T1和T2是否相等
        if (row.T1 !== row.T2 && row.T1 !== '' && row.T2 !== '') {
          // 如果Track1有数据则使用蓝色背景，否则使用红色警告
          if (row.Track1 && row.Track1 !== '') {
            return 'info-cell' // 使用浅蓝色背景
          } else {
            return 'warning-cell' // 继续使用淡红色警告
          }
        }
      }
    ...
    exportResultToExcel(calculationType) {
      ...
      // 获取当前选中行的索引
      const rowIndex = this.selectedCalculationRow ? this.selectedCalculationRow.index : 0

      // 加载running_profile_up_slow.json文件并显示对应索引的数据
      fetch('/data/result/running_profile_up_slow.json')
        ...
        .then(data => {
          if (data && Array.isArray(data) && rowIndex < data.length) {
            ...
            // 创建HTML表格来展示JSON数据
            let htmlContent = '<table class="json-table">'
            htmlContent += '<tbody>'
            ...
            // 使用Element UI的MessageBox显示数据
            this.$confirm(htmlContent, '运行配置数据', {
              ...
              // 用户点击下载Excel按钮
              this.downloadExcelFile(jsonData)
            }).catch(() => {
              // 用户取消
            })
          } else {
            this.$message.error('未找到对应的数据或数据格式不正确')
          }
        })
        .catch(error => {
          console.error('加载JSON文件失败:', error)
          this.$message.error('加载JSON文件失败')
        })
    ...
    // 添加下载Excel文件的方法
    downloadExcelFile(jsonData) {
      try {
        // 创建工作簿
        import('@/vendor/Export2Excel').then(excel => {
          // 准备表头和数据
          const tHeader = Object.keys(jsonData)
          const data = [Object.values(jsonData)]

          // 生成Excel文件并下载
          const filename = `运行配置_${new Date().getTime()}.xlsx`
    ...
    exportResultToImage(calculationType) {
      ...
      // 对于通行时间，继续原有的图片展示逻辑
      // 加载running_profile_up_slow.json文件获取对应的站点信息
      fetch('/data/result/running_profile_up_slow.json')
        .then(response => response.json())
        .then(data => {
          if (data && Array.isArray(data) && rowIndex < data.length) {
            ...
            const arrivalStationNoDigit = arrivalStation.replace(/\d+/g, '')
            const imageName = `${departureStationNoDigit}-${arrivalStationNoDigit}.png`

            // 构建图片路径
            const imagePath = `/data/result/tongxing_time/${imageName}`
            ...
            fetch(imagePath)
              .then(response => {
                if (response.ok) {
                  // 显示图片预览
                  this.showImagePreview(imagePath, departureStation, arrivalStation)
                } else {
                  // 尝试使用原始命名格式（带数字）
                  const originalImageName = `${departureStation}-${arrivalStation}.png`
                  const originalImagePath = `/data/result/tongxing_time/${originalImageName}`

                  // 再次尝试获取图片
                  fetch(originalImagePath)
                    .then(response => {
                      if (response.ok) {
                        this.showImagePreview(originalImagePath, departureStation, arrivalStation)
                      } else {
    ...
    // 生成详情数据 - 从JSON文件加载真实数据
    generateRandomDetailData() {
      // 获取当前选中行的数据
      const currentRow = this.selectedCalculationRow ? this.selectedCalculationRow.data : this.selectedRow
      if (!currentRow) {
        console.error('未找到行数据，无法生成详情')
        return
      }

      // 获取出发地和目的地（原始可能包含数字1）
      const originWithDigit = currentRow['出发地'] || currentRow['Departure_Station_name'] || currentRow['departureStation'] || '起点'
      const destinationWithDigit = currentRow['目的地'] || currentRow['Arrival_Station_name'] || currentRow['arrivalStation'] || '终点'

      // 去除站名中的数字"1"
      const origin = originWithDigit.replace(/1/g, '')
      const destination = destinationWithDigit.replace(/1/g, '')

      const folderName = `${origin}-${destination}的时间间隔数据以及图片、`
      console.log(`详情数据路径: ${folderName}`)

      // 基础路径
      const basePath = `/data/result/min_time/${folderName}/`

      const jsonPath = `${basePath}xjwb.json`

      // 初始化加载状态
      this.detailData = this.initDetailLoadingState()
      ...
      this.detailData.module1 = {
              ...
              time3: this.formatDetailValue(data.departure.departureRouteTime) + ' 秒',
              interval: this.formatDetailValue(departureTotal) + ' 秒'
            }
          }
          ...
          if (data.blocks && Array.isArray(data.blocks)) {
            this.detailData.module2 = data.blocks.map((block, index) => {
              ...
              return {
                ...
                startPoint: this.formatDetailValue(block.blockStartDistance) + ' m',
                endPoint: this.formatDetailValue(block.blockEndDistance) + ' m',
                bottleneckPosition: this.formatDetailValue(block.blockBottleneckPoint) + ' m',
                clearPosition: this.formatDetailValue(block.blockClearPoint) + ' m',
                dangerPosition: this.formatDetailValue(block.blockDangerPoint) + ' m',
                bottleneckToClearTime: this.formatDetailValue(block.timeFromBottleneckToBlockClear) + ' 秒',
          ...
          if (data.arriver) {
            ...
            this.detailData.module3 = {
              ...
              entranceBottleneck: this.formatDetailValue(data.arriver.blockBottleneckPoint) + ' m',
              stopPoint: this.formatDetailValue(data.arriver.stationStopPoint) + ' m',
              platformStopTime: this.formatDetailValue(data.arriver.stionStopTime) + ' 秒',
              frontTrainToClearTime: this.formatDetailValue(data.arriver.blockClearPoint) + ' 秒',
              clearDelayTime: this.formatDetailValue(data.arriver.timeClearDelay) + ' 秒',
              receiveTrainTime: this.formatDetailValue(data.arriver.arriverRouteTime) + ' 秒',
              rearTrainToStopTime: this.formatDetailValue(data.arriver.timeFromBottleneckToStop) + ' 秒',
              trainInterval: this.formatDetailValue(arriverTotal) + ' 秒'
            }
          }
        })
        .catch(error => {
          console.error('加载JSON数据失败:', error)
          this.$message.error(`无法加载数据: ${error.message}`)
    ...
    showImagePreview(imagePath, departureStation, arrivalStation) {
      // 在Vue实例中添加图片信息
      this.previewImageInfo = {
        path: imagePath,
        title: `${departureStation}到${arrivalStation}运行图`,
        fileName: `${departureStation}-${arrivalStation}.png`
      }
    ...
    handleSingleRowCalculate(row, index) {
      ...
      if (this.hasCalculationResults(row, index)) {
        ...
        if (this.calculatedRows[rowId] && this.calculatedRows[rowId].travelTime) {
          const travelTime = this.calculatedRows[rowId].travelTime
      ...
      // 如果全局已经计算过通行时间，则更新状态和启用最小间隔时间按钮
    ...
    calculateTravelTimeForRow() {
      ...
      const index = this.selectedCalculationRow.index
      const rowId = this.getRowId(row)
      ...
      const travelTime = travelTimes[index] || 200.00
    ...
    updateStartT1(index) {
      if (this.activeSheet === 'Block description') {
        ...
        const nextIndex = matchIndex + 1
        if (nextIndex < tracksData.length) {
          currentRow.Start_T1 = tracksData[nextIndex].Track_ID_before_jump
        } else {
          currentRow.Start_T1 = tracksData[matchIndex].Track_ID_before_jump // 如果已经是最后一行，使用当前行
        }
    ...
    viewImage(module, index) {
      ...
      console.log(`构建图片路径: 原始站名: ${originWithDigit}-${destinationWithDigit}, 处理后: ${origin}-${destination}`)
    ...
    // 检查区间模块图片数量
    checkBlockImagesCount() {
      return new Promise((resolve) => {
        // 获取当前选中行的数据
        const currentRow = this.selectedCalculationRow ? this.selectedCalculationRow.data : this.selectedRow
        if (!currentRow) {
          console.error('未找到行数据，无法检查区间图片')
          resolve(0)
          return
        }
        ...
        // 检查每个可能的文件
        const checkNextFile = (index) => {
          if (index > maxIndex) {
            // 已检查完所有可能的文件
            console.log(`发现${foundFiles}个区间图片`)
            resolve(foundFiles || 1) // 至少返回1个，确保界面不为空
            return
          }

          const imagePath = `${basePath}block-${index}.png`
        ...
        .then(response => {
              ...
              if (response.ok) {
                // 文件存在
                foundFiles++
                console.log(`找到文件: block-${index}.png`)
              }

              // 继续检查下一个文件
              checkNextFile(index + 1)
            })
            .catch(() => {
              // 出错了，可能是文件不存在
              checkedCount++
              console.log(`未找到文件: block-${index}.png`)
        ...
        // 开始检查第一个文件，从block-1.png开始
        checkNextFile(startIndex)
      })
    },
    ...
    exportExcel(module, index) {
      ...
      if (module === 1) {
        ...
        exportData = {
          ...
          '发车点至出清点时间(秒)': this.detailData.module1.time1.replace(' 秒', ''),
          '出清延迟时间(秒)': this.detailData.module1.time2.replace(' 秒', ''),
          '发车进路办理时间(秒)': this.detailData.module1.time3.replace(' 秒', ''),
          '发车时间间隔(秒)': this.detailData.module1.interval.replace(' 秒', '')
        }
        filename = `发车模块_${origin}-${destination}_${new Date().getTime()}`
      } else if (module === 2) {
        // 区间模块数据，使用index参数获取具体的区间数据
        if (index !== undefined && this.detailData.module2[index]) {
          const blockData = this.detailData.module2[index]
          exportData = {
            '模块类型': '区间模块',
            '区间': `${origin}-${destination}`,
            '区间序号': index + 1,
          ...
          filename = `区间模块${index + 1}_${origin}-${destination}_${new Date().getTime()}`
        } else {
          this.$message.error('未找到区间数据')
          return
        }
      } else if (module === 3) {
        // 接车模块数据
        exportData = {
          '模块类型': '接车模块',
          '区间': `${origin}-${destination}`,
          '进站瓶颈点(m)': this.detailData.module3.entranceBottleneck.replace(' m', ''),
          '停车点(m)': this.detailData.module3.stopPoint.replace(' m', ''),
          '站台停车时间(秒)': this.detailData.module3.platformStopTime.replace(' 秒', ''),
          '前车从发车点至股道出清点时间(秒)': this.detailData.module3.frontTrainToClearTime.replace(' 秒', ''),
          '出清延迟时间(秒)': this.detailData.module3.clearDelayTime.replace(' 秒', ''),
          '接车进路办理时间(秒)': this.detailData.module3.receiveTrainTime.replace(' 秒', ''),
          '后车从瓶颈点至停车点时间(秒)': this.detailData.module3.rearTrainToStopTime.replace(' 秒', ''),
          '接车时间间隔(秒)': this.detailData.module3.trainInterval.replace(' 秒', '')
        }
        filename = `接车模块_${origin}-${destination}_${new Date().getTime()}`
      }
      ...
      try {
        import('@/vendor/Export2Excel').then(excel => {
          // 准备表头和数据
          const tHeader = Object.keys(exportData)
    ...
    downloadImageFromPreview() {
      this.downloadImage(this.previewImageInfo.path, this.previewImageInfo.fileName)
    ...
    handleCalculateCommand(command) {
      if (command === 'travelTime') {
        ...
        const updateSingleRow = () => {
            ...
            this.$notify({
              ...
              message: h('div', null, [
                ...
                h('el-button', {
                  ...
                  on: {
                    click: () => this.downloadAllExcelsAsSingleFile()
                  }
                }, '下载合并Excel'),
                h('el-button', {
                  props: {
                  ...
                  on: {
                    click: () => this.downloadAllImagesAsZip()
                  }
                }, '下载图片ZIP包')
              ]),
              duration: 0,
          ...
          // 获取当前行的计算时间
          const travelTime = travelTimes[currentIndex] || 200.00
        ...
        rows.forEach((row, index) => {
          ...
          this.$set(this.calculatedRows, rowId, {
            calculating: true,
            calculated: false,
            minHeadwayCalculating: true // 标记最小间隔时间正在计算
          })
        })
        ...
        const updateSingleRow = () => {
            ...
            this.$notify({
              ...
              message: h('div', null, [
                ...
                h('el-button', {
                  ...
                  on: {
                    click: () => this.downloadAllFilesAsZip()
                  }
                }, '下载Excel和图片')
              ]),
              duration: 0,
          ...
          setTimeout(() => {
            ...
            this.$set(this.calculatedRows, rowId, {
              calculating: false,
              calculated: true,
              minHeadwayCalculated: true, // 标记最小间隔时间已计算
              minHeadwayTime: Math.floor(Math.random() * 200 + 100) // 模拟一个最小间隔时间值
            })
        ...
        // 开始第一行的计算
        updateSingleRow()
      } else if (command === 'downloadExcels') {
        // 下载所有Excel（多个文件）
        this.downloadAllExcelFiles()
      } else if (command === 'downloadMergedExcel') {
        // 下载所有Excel（合并为一个文件）
        this.downloadAllExcelsAsSingleFile()
      } else if (command === 'downloadImages') {
        // 下载所有图片（多个文件）
        this.downloadAllImages()
      } else if (command === 'downloadImagesZip') {
        // 下载所有图片（ZIP包）
      ...
      Promise.all(
        ...
        try {
          ...
          import('@/vendor/Export2Excel').then(excel => {
            ...
            if (allData.length > 0) {
              // 使用第一行数据获取所有可能的表头
              const tHeader = Object.keys(allData[0]).filter(key => key !== '_rowLabel')

              // 为每一行准备数据
              const data = allData.map(item => {
                return tHeader.map(key => item[key])
              })

              // 生成Excel文件并下载
              const filename = `合并运行配置_${new Date().getTime()}`
              excel.export_json_to_excel({
                header: tHeader,
                data,
                filename,
              ...
              this.$message.success('合并Excel文件下载成功')
            } else {
          ...
          this.$message.error('合并Excel文件失败: ' + error.message)
        }
      })
    },
    ...
    downloadAllImagesAsZip() {
      ...
      import('jszip').then(({ default: JSZip }) => {
        const zip = new JSZip()
        // 创建文件夹
        const tongxingFolder = zip.folder('通行时间图片')
        const minHeadwayFolder = zip.folder('最小间隔时间图片')

        // 加载计数器
        let loadedCount = 0
        const totalRows = rows.length

        // 创建一个Promise数组用于跟踪所有图片的加载
                  ...
                  fetch(imagePath)
                    .then(response => {
                      if (response.ok) {
                        ...
                        const originalImageName = `${departureStation}-${arrivalStation}.png`
                        const originalImagePath = `/data/result/tongxing_time/${originalImageName}`
                        return fetch(originalImagePath).then(resp => {
                          if (resp.ok) {
                            return resp.blob()
                          }
                    ...
                    .then(blob => {
                      // 将通行时间图片添加到ZIP文件
                      const fileName = `${departureStation}-${arrivalStation}_通行时间.png`
                      tongxingFolder.file(fileName, blob)
                    ...
                    .finally(() => {
                      // 最小间隔时间图片 - 尝试加载发车、区间和接车模块图片
                      const folderName = `${departureStationNoDigit}-${arrivalStationNoDigit}的时间间隔数据以及图片`
                      const minHeadwayBasePath = `/data/result/min_time/${folderName}/`

                      // 加载发车模块图片
                      fetch(`${minHeadwayBasePath}发车.png`)
                        .then(response => {
                          if (response.ok) return response.blob()
                          throw new Error('发车图片不存在')
                        })
                        .then(blob => {
                          const fileName = `${departureStation}-${arrivalStation}_发车模块.png`
                          minHeadwayFolder.file(fileName, blob)
                          loadedCount++
                        })
                        .catch(() => {})

                      // 加载接车模块图片
                      fetch(`${minHeadwayBasePath}接车.png`)
                        .then(response => {
                          if (response.ok) return response.blob()
                          throw new Error('接车图片不存在')
                        })
                        .then(blob => {
                          const fileName = `${departureStation}-${arrivalStation}_接车模块.png`
                          minHeadwayFolder.file(fileName, blob)
                          loadedCount++
                        })
                        .catch(() => {})
                      ...
                      for (let blockIndex = 1; blockIndex <= 10; blockIndex++) {
                        blockPromises.push(
                          fetch(`${minHeadwayBasePath}block-${blockIndex}.png`)
                            .then(response => {
                              if (response.ok) {
                                return response.blob().then(blob => {
                                  const fileName = `${departureStation}-${arrivalStation}_区间模块${blockIndex}.png`
                                  minHeadwayFolder.file(fileName, blob)
                                  loadedCount++

                                  // 通知用户进度
                                  if (loadedCount % 5 === 0 || loadedCount === totalRows * 4) {
                                    this.$message({
                                      type: 'info',
                                      message: `已处理 ${loadedCount} 个图片文件...`
                                    })
                                  }
                                })
                              }
                              throw new Error(`区间图片${blockIndex}不存在`)
                            })
                            .catch(() => {})
                        )
                      }
        ...
        Promise.all(promises).then(() => {
          ...
          zip.generateAsync({ type: 'blob' })
            .then(content => {
              ...
              const link = document.createElement('a')
              link.href = url
              link.download = `运行图片_${new Date().getTime()}.zip`
              document.body.appendChild(link)
              link.click()
              document.body.removeChild(link)
              window.URL.revokeObjectURL(url)
        ...
        this.$message.error('加载打包库失败，请确保已安装JSZip')
      })
    },

    // 添加下载所有Excel文件的方法
    downloadAllExcelFiles() {
      // 获取当前表格中的所有行
      const rows = this.currentSheets[this.activeSheet].data
      ...
      this.$message({
        type: 'info',
        message: '正在准备下载所有Excel文件，请稍候...'
      })

      // 创建一个延迟队列，避免同时下载过多文件导致浏览器卡顿
      Promise.all(
        rows.map((row, index) => {
          return new Promise(resolve => {
            setTimeout(() => {
            ...
            }, index * 500) // 每500毫秒下载一个文件
          })
        })
      ).then(() => {
        this.$message({
          type: 'success',
          message: '所有Excel文件下载任务已完成'
        })
      })
    },
      ...
      Promise.all(
        rows.map((row, index) => {
          return new Promise(resolve => {
                  ...
                  if (data && Array.isArray(data) && index < data.length) {
                    ...
                    fetch(imagePath)
                      .then(response => {
                        if (response.ok) {
                          // 下载图片
                          this.downloadImage(imagePath, imageName)
                        } else {
                          // 尝试使用原始命名格式（带数字）
                          const originalImageName = `${departureStation}-${arrivalStation}.png`
                          const originalImagePath = `/data/result/tongxing_time/${originalImageName}`
                          ...
                          fetch(originalImagePath)
                            .then(response => {
                              if (response.ok) {
                                this.downloadImage(originalImagePath, originalImageName)
                              }
                            })
                            .catch(() => {})
                        }
            ...
            }, index * 500) // 每500毫秒处理一个文件
          })
        })
      ).then(() => {
        this.$message({
          type: 'success',
          message: '所有图片文件下载任务已完成'
        })
      })
    },
    ...
    initDetailLoadingState() {
      return {
        ...
        module2: [{
          ...
          startPoint: '加载中...',
          endPoint: '加载中...',
          bottleneckPosition: '加载中...',
          clearPosition: '加载中...',
          dangerPosition: '加载中...',
          bottleneckToClearTime: '加载中...',
          clearDelayTime: '加载中...',
          sectionInterval: '加载中...'
        }],
        module3: {
          intervalTitle: '间隔值: 加载中...',
          entranceBottleneck: '加载中...',
          stopPoint: '加载中...',
          platformStopTime: '加载中...',
          frontTrainToClearTime: '加载中...',
          clearDelayTime: '加载中...',
          receiveTrainTime: '加载中...',
          rearTrainToStopTime: '加载中...',
          trainInterval: '加载中...'
        }
      }
    }
...
.sheet-select >>> .el-input__inner {
  height: 32px;
  line-height: 32px;
}

.sheet-select >>> .el-select-dropdown__item.selected {
  color: #409EFF;
  font-weight: bold;
}
...
.sheet-toolbar .toolbar-left {
  flex: 1;
  display: flex;
  align-items: center;
}

.sheet-toolbar .toolbar-label {
  margin-right: 10px;
  font-weight: bold;
  white-space: nowrap;
}

.sheet-toolbar .sheet-select {
  width: 180px;
  margin-right: 15px;
}

.sheet-toolbar .insert-btn {
  margin-left: 10px;
  white-space: nowrap;
}
...
.el-table >>> .numeric-cell .cell {
  overflow: visible;
}

.el-table >>> .numeric-cell .el-input__inner {
  text-align: right;
...
.el-table >>> .disabled-cell .el-input__inner {
  background-color: #f5f7fa;
  color: #606266;
}
...
/* 添加Track警告样式 */
.el-table >>> .warning-cell .el-input__inner {
  background-color: #fef0f0 !important; /* 淡红色背景 */
  border-color: #fbc4c4 !important; /* 红色边框 */
  color: #f56c6c !important;
}

.el-table >>> .warning-cell.cell {
  position: relative;
}

.el-table >>> .warning-cell.cell::after {
  content: '!';
  position: absolute;
  top: 2px;
  right: 2px;
...
/* 添加info-cell样式，确保应用于input元素 */
.el-table >>> .info-cell .el-input__inner {
  background-color: #e6f1fc !important; /* 浅蓝色背景 */
  border-color: #a3d0fd !important; /* 蓝色边框 */
...
/* 添加JSON预览对话框样式 */
.json-preview-dialog >>> .el-message-box__content {
  max-height: 70vh;
  overflow-y: auto;
...
.json-preview-dialog >>> .el-message-box {
  width: 600px;
  max-width: 90vw;
}

/* 添加表格样式 */
.json-preview-dialog >>> .json-table {
  width: 100%;
...
.json-preview-dialog >>> .json-table tr:nth-child(odd) {
  background-color: #f8f8f9;
}

.json-preview-dialog >>> .json-table tr:hover {
  background-color: #ecf5ff;
}

.json-preview-dialog >>> .key-cell {
  width: 40%;
  padding: 8px 12px;
  border: 1px solid #ebeef5;
  font-weight: bold;
...
.json-preview-dialog >>> .value-cell {
  width: 60%;
  padding: 8px 12px;
  border: 1px solid #ebeef5;
  color: #303133;
...
/* 覆盖Element UI对话框样式 */
.full-screen-image-dialog >>> .el-dialog__body {
  padding: 0;
  height: calc(100vh - 120px);
  overflow: auto;
}
...
.full-screen-image-dialog >>> .el-dialog__footer {
  padding: 10px 20px;
}
.el-table .cell.info-cell {
